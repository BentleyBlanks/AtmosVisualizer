	...
	void Dockspace::updateAndDraw(ImVec2 dockspaceSize)
	{
		...
		std::function<void(Container*, ImVec2, ImVec2)> renderContainer = [&](Container *container, ImVec2 size, ImVec2 cursorPos) {
			...
			if (container->splits[0] == nullptr && container != &m_container)
			{
				...
				ImGui::PushStyleColor(ImGuiCol_ChildWindowBg, ImVec4(.28, .28, .32, 1));
    ...
    */
	...
	void Dockspace::_renderTabBar(Container *container, ImVec2 size, ImVec2 cursorPos)
	{
		...
		for (auto dock : container->docks)
		{
			...
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.28, .28, .32, 1));
				ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(.28, .28, .32, 1));
				ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.28, .28, .32, 1));
			}
			else
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.28, .28, .32, .6));
				ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(.35, .35, .39, .6));
				ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.4, .4, .44, .6));
			}
